...
namespace AnySilkBoss.Source.Managers
{
    ...
    internal class SaveSwitchManager : MonoBehaviour
    {
        ...
        private const string BOSS_SAVE_FILE = "AnySilkBoss.dat";
        ...
        private void Update()
        {
            ...
            if (Plugin.IsInBossRoom && _isCheckingAudio && !_hasSwitchedThisSession)
            {
        ...
        private IEnumerator SwitchSaveFile()
        {
            ...
            // 检查当前存档是否是Boss存档
            bool isCurrentlyBossSave = IsBossSave(userSavePath);

            Log.Info($"当前存档状态检测 - 是否是Boss存档: {isCurrentlyBossSave}");
            // 在切换存档前禁用用户操作0.5秒
            yield return DisablePlayerInput();
            if (isCurrentlyBossSave)
            {
                // 如果当前是Boss存档，切换回原存档
                Log.Info("切换回原存档");
            ...
            else
            {
                // 如果当前是原存档，切换到Boss存档
                Log.Info("切换到Boss存档");
                yield return SwitchToBossSave();
            }
            // 重新启用用户操作
            yield return EnablePlayerInput();
        }
        private bool IsBossSave(string savePath)
        {
            try
            {
                ...
                // 通过检查备份文件是否存在来判断 如果备份存在，说明当前是Boss存档
        ...
        private IEnumerator SwitchToBossSave()
        {
            Log.Info("开始切换到Boss存档");

            // 获取Boss存档文件路径
            string bossSavePath = GetBossSaveFilePath();
            if (string.IsNullOrEmpty(bossSavePath) || !File.Exists(bossSavePath))
            {
                Log.Error($"Boss存档文件不存在: {bossSavePath}");
                yield break;
            }
            ...
            Log.Info($"Boss存档路径: {bossSavePath}");
            ...
            // 直接替换存档文件（原存档会被覆盖）
            File.Copy(bossSavePath, userSavePath, true);
            Log.Info($"Boss存档已复制到用户存档位置");
            // 创建重生标记
            CreateRespawnMarker();
            ...
            Log.Info("Boss存档加载完成，当前状态：Boss存档已激活");
        }
        ...
        private string GetBossSaveFilePath()
        {
            try
            {
                ...
                string resourceName = $"AnySilkBoss.Assets.{BOSS_SAVE_FILE}";
