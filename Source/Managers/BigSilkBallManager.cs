...
namespace AnySilkBoss.Source.Managers
{
    ...
    internal class BigSilkBallManager : MonoBehaviour
    {
        ...
        private void ProcessChildObjects()
        {
            ...
            // tendrils(5层) - 触须
            // loom_threads(2层) - 织网线
            // Audio Heartbeat - 心跳音效

            Log.Info("保留所有子物体及其组件");

            // 记录所有子物体
            foreach (Transform child in _bigSilkBallPrefab.transform)
            {
                Log.Info($"  子物体保留: {child.name}");
            }
        }
        #endregion
        ...
        public GameObject? SpawnBigSilkBall(Vector3 position, GameObject bossObject)
        {
            ...
            bigSilkBall.transform.position = position;
            bigSilkBall.SetActive(true);

            Log.Info($"生成大丝球到位置: {position}");

            // 确保 over_glow 子物体的发光效果正常
            EnsureOverGlowEffect(bigSilkBall);
            ...
            // 初始化行为
            behavior.Initialize(bossObject);
        ...
        /// 修复子物品的Z轴位置（根据原版数据）
        /// </summary>
        private void FixChildrenZPosition(GameObject bigSilkBall)
        {
            // 原版数据：
            // 根物品绝对位置: (49.5809, 132.281, 57.4491)
            // heart相对位置: (-9.4, -2.9, 0.0214)
            // over_glow相对位置: (-10.0809, -18.781, -52.56)
            
            // 根物品的Z轴保持为0（在2D游戏中）
            var rootPos = bigSilkBall.transform.position;
            rootPos.z = 0.0214f;  // 使用接近原版heart的相对Z轴
            bigSilkBall.transform.position = rootPos;

            // 修复所有子物品和孙物品的Z轴
            FixChildZRecursive(bigSilkBall.transform, 0);
        }

        /// <summary>
        /// 递归修复子物品的Z轴位置
        /// </summary>
        private void FixChildZRecursive(Transform parent, int depth)
        {
            foreach (Transform child in parent)
            {
                ...
                if (child.name == "heart")
                {
                    // heart的相对Z轴应该是0.0214
                    localPos.z = 0.0214f;
                    Log.Info($"{indent}修复 {child.name} 的Z轴: 本地Z从 {child.localPosition.z:F4} 改为 {localPos.z:F4}");
                }
                else if (child.name == "over_glow")
                {
                    // over_glow的相对Z轴应该是-52.56
                    localPos.z = -52.56f;
                    Log.Info($"{indent}修复 {child.name} 的Z轴: 本地Z从 {child.localPosition.z:F4} 改为 {localPos.z:F4}");
                }
                else if (child.name == "burst")
                {
                    // burst通常应该在heart前面一点
                    localPos.z = 0.05f;
                    Log.Info($"{indent}修复 {child.name} 的Z轴: 本地Z从 {child.localPosition.z:F4} 改为 {localPos.z:F4}");
                ...
                else
                {
                    ...
                    {
                        float oldZ = localPos.z;
                        localPos.z = 0f;
                        Log.Info($"{indent}重置 {child.name} 的Z轴: 本地Z从 {oldZ:F4} 改为 0");
                    }
                }
                ...
                {
                    FixChildZRecursive(child, depth + 1);
                }
            }
        }
        ...
        /// 确保over_glow的发光效果正常
        /// </summary>
        private void EnsureOverGlowEffect(GameObject bigSilkBall)
        {
            var overGlow = bigSilkBall.transform.Find("over_glow");
            if (overGlow != null)
            {
                // 检查 NestedFadeGroupSpriteRenderer 组件
                var nestedFade = overGlow.GetComponent("NestedFadeGroupSpriteRenderer");
                if (nestedFade != null)
                {
                    // 使用反射设置AlphaSelf为1（完全不透明）
                    var type = nestedFade.GetType();
                    var alphaSelfProperty = type.GetProperty("AlphaSelf");
                    if (alphaSelfProperty != null)
                    {
                        alphaSelfProperty.SetValue(nestedFade, 1f);
                        Log.Info("已设置 over_glow 的 AlphaSelf = 1");
                    }
                }

                // 确保 SpriteRenderer 的颜色alpha为1
                var spriteRenderer = overGlow.GetComponent<SpriteRenderer>();
                if (spriteRenderer != null)
                {
                    ...
                    Log.Info($"已设置 over_glow 的 SpriteRenderer.color.a = 1，enabled = true");
                }
            }
            else
            {
                Log.Warn("未找到 over_glow 子物体");
            }
        }
